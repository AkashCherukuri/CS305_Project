...
uint64_t warmup_instructions = 50000000, simulation_instructions = 10000000,
         champsim_seed, occupancy_zero_cycle;

extern uint64_t variantx_evicty[][4];
extern uint64_t sizeofblock[];
extern int reg_instruction_pointer, reg_stack_pointer,
    reg_flags; // From src/ooo_cpu.cc

extern map<uint64_t, map<uint64_t, set<uint64_t>>> offsets_per_branch_type;

extern map<uint64_t, set<uint64_t>> btb_unique_addr;
extern map<uint64_t, uint64_t> btb_fill_count;

extern map<uint64_t, set<uint64_t>> unique_branch_target;

extern map<uint64_t, set<uint64_t>> unique_trigger_per_target;

extern map<uint64_t, set<uint64_t>> offset_bits_required, ip_per_tag;

extern uint64_t total_stall_cycle_per_branch[];
extern uint64_t total_fetch_cycle_per_branch[];
extern uint64_t total_fetch_queue_cycle_per_branch[];
extern uint64_t total_decode_cycle_per_branch[];
extern uint64_t total_execute_cycle_per_branch[];
extern uint64_t total_stall_count[];

extern set<uint64_t> upper_32_bits, upper_36_bits, upper_40_bits, upper_44_bits,
    upper_48_bits;

extern map<uint64_t, uint64_t> dispatches_per_cycle;
extern uint64_t rob_full_cycles;
uint64_t decode_empty_cycles = 0, decode_dispatch_called = 0;

#if defined(MICRO_BTB)
extern unordered_map<uint64_t, unordered_set<uint64_t>> variant1, variant2,
    variant3;
extern unordered_map<uint64_t, unordered_set<uint64_t>> variant1_per_set[],
    variant2_per_set[], variant3_per_set[];
extern uint64_t correct_prediction[], incorrect_prediction[];
#endif

#ifdef SHOTGUN_BTB
extern set<uint64_t> ubtb_set_used, cbtb_set_used, rib_set_used;

extern set<uint64_t> ubtb_unique_addr[UBTB_SET], cbtb_unique_addr[CBTB_SET],
    rib_unique_addr[RIB_SET];
extern uint64_t ubtb_fill_count[UBTB_SET], cbtb_fill_count[CBTB_SET],
    rib_fill_count[RIB_SET];
#endif

uint64_t total_rob_occupancy = 0, total_decode_occupancy = 0,
         total_ftq_occupancy = 0;

extern unordered_map<uint64_t, uint64_t>
    branch_type_per_ip; // From src/ooo_cpu.cc
extern set<uint64_t> instruction_pages;
extern set<uint64_t> data_pages;
extern unordered_map<uint64_t, pair<uint64_t, uint64_t>>
    reuse_distance_per_branch;

extern unordered_map<uint64_t, unordered_set<uint64_t>> branch_basic_block_size;

#ifdef SHOTGUN_BTB
extern uint64_t to_predecode;
extern uint64_t predecoding_something, conditional_predecode,
    direct_call_predecode, direct_branch_predecode;
...
void btb_record_roi_stats(uint32_t cpu, SHOTGUN *btb) {
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    btb->roi_access[cpu][i] = btb->sim_access[cpu][i];
    btb->roi_hit[cpu][i] = btb->sim_hit[cpu][i];
    btb->roi_miss[cpu][i] = btb->sim_miss[cpu][i];
    btb->roi_miss_target[cpu][i] = btb->sim_miss_target[cpu][i];
  }
}
#endif
...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
  ...
  if (cache->cache_type == IS_BTB) {
    ...
    for (int i = 0; i < 7; i++) {
      ...
      cout << type[i] << setw(10) << cache->roi_access[cpu][i]
           ...
           << " MISS: " << setw(10) << cache->roi_miss[cpu][i]
           << " AVERAGE STALL: "
           << (total_stall_cycle_per_branch[i + 1] * 1.0) /
                  total_stall_count[i + 1]
           << " AVERAGE FETCH STALL: "
           << (total_fetch_cycle_per_branch[i + 1] * 1.0 /
               total_stall_count[i + 1])
           << " AVERAGE FETCH QUEUE STALL: "
           << (total_fetch_queue_cycle_per_branch[i + 1] * 1.0 /
               total_stall_count[i + 1])
           << " AVERAGE DECODE STALL: "
           << (total_decode_cycle_per_branch[i + 1] * 1.0 /
               total_stall_count[i + 1])
           << " AVERAGE EXECUTE STALL: "
           << (total_execute_cycle_per_branch[i + 1] * 1.0 /
               total_stall_count[i + 1])
           << endl;
    }
    cout << cache->NAME;
    ...
    cout << " TOTAL ROB OCCUPANCY AT BTB MISS: "
         << (1.0 * (ooo_cpu[cpu].total_rob_occupancy_at_btb_mispredict) /
             TOTAL_MISS)
         << endl;
  ...
  string type[] = {" LOAD ",
                   ...
                   " TRANSLATION FROM L1D PREFETCHER ",
                   " LOAD ",
                   " RFO ",
                   " PREFETCH ",
                   " WRITEBACK ",
  ...
  for (int i = 0; i < 14; i++) {
    if (!cache->roi_access[cpu][i])
      continue;

    cout << cache->NAME;
    if (i < 7)
      cout << type[i] << "INSTRUCTION	ACCESS: ";
    else
      cout << type[i] << "DATA	ACCESS: ";

    cout << setw(10) << cache->roi_access[cpu][i] << "  HIT: " << setw(10)
         << cache->roi_hit[cpu][i] << "  MISS: " << setw(10)
         << cache->roi_miss[cpu][i] << "  HIT %: " << setw(10)
         << ((double)cache->roi_hit[cpu][i] * 100 / cache->roi_access[cpu][i])
         << "  MISS %: " << setw(10)
         << ((double)cache->roi_miss[cpu][i] * 100 / cache->roi_access[cpu][i])
         << "  MPKI: " << ((double)cache->roi_miss[cpu][i] * 1000 / num_instrs)
         << endl;
  }
  cout << endl;
  ...
  if (cache->cache_type == IS_L1D) {
    cout << cache->NAME
         << " UNIQUE REGIONS ACCESSED: " << cache->unique_region_count << endl;
    cout << cache->NAME << " REGIONS CONFLICTS: " << cache->region_conflicts
         << endl;
    cout << cache->NAME << " Cross Page Prefetch Requests: "
         << cache->cross_page_prefetch_requests << endl;
    cout << cache->NAME << " Same Page Prefetch Requests: "
         << cache->same_page_prefetch_requests << endl;

    cout << cache->NAME
         << " ROI Sum of L1D PQ occupancy: " << cache->sum_pq_occupancy << endl;
    cout << cache->NAME
         << " PREFETCHES PUSHED FROM L2C: " << cache->pf_pushed_from_L2C
         << endl;
  }

  if (cache->cache_type == IS_STLB) {
    cout << cache->NAME << " Hit, L1D data hit: " << cache->l1d_data_hit
         << endl;
    cout << cache->NAME << " Hit, L2C data hit: " << cache->l2c_data_hit
         << endl;
    cout << cache->NAME << " Hit, LLC data hit: " << cache->llc_data_hit
         << endl;
    cout << cache->NAME << " Hit, LLC data miss: " << cache->llc_data_miss
         << endl;
    cout << cache->NAME << " STLB hints to L2: " << cache->stlb_hints_to_l2
         << endl;
  }

  if (cache->cache_type == IS_L2C)
    cout << cache->NAME
         << " Dense regions hint from L2: " << cache->getting_hint_from_l2
         << endl;
  if (cache->cache_type == IS_LLC)
    cout << cache->NAME
         << " Dense regions hint to LLC: " << cache->sending_hint_to_llc
         << endl;
  ...
  if (cache->cache_type == IS_DTLB_PB && flag == 0) {
    flag = 1;
    cout << "DTLB_PB PREFETCH USEFUL - " << cache->pf_useful
         << " USELESS PREFETCHES: " << cache->pf_useless << endl;
  }
#endif
}

#if defined(SHOTGUN_BTB)
void btb_print_roi_stats(uint32_t cpu, SHOTGUN *btb) {
  uint64_t TOTAL_ACCESS = 0, TOTAL_HIT = 0, TOTAL_MISS = 0,
           TOTAL_MISS_TARGET = 0;
  uint64_t num_instrs = ooo_cpu[cpu].finish_sim_instr;

  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    TOTAL_ACCESS += btb->roi_access[cpu][i];
    TOTAL_HIT += btb->roi_hit[cpu][i];
    TOTAL_MISS += btb->roi_miss[cpu][i];
    TOTAL_MISS_TARGET += btb->roi_miss_target[cpu][i];
  }

  cout << "BTB";
  cout << " TOTAL     ACCESS: " << setw(10) << TOTAL_ACCESS
       << "  HIT: " << setw(10) << TOTAL_HIT << "  MISS: " << setw(10)
       << TOTAL_MISS << "  MISS TARGET: " << setw(10) << TOTAL_MISS_TARGET
       << "  HIT %: " << setw(10) << ((double)TOTAL_HIT * 100 / TOTAL_ACCESS)
       << "  MISS %: " << setw(10) << ((double)TOTAL_MISS * 100 / TOTAL_ACCESS)
       << "   MPKI: " << ((double)TOTAL_MISS * 1000 / num_instrs) << endl;
  ...
  for (int i = 0; i < 7; i++) {
    cout << "BTB";
    cout << type[i] << setw(10) << btb->roi_access[cpu][i]
         << "  HIT: " << setw(10) << btb->roi_hit[cpu][i]
         << "  MISS: " << setw(10) << btb->roi_miss[cpu][i]
         << "  MISS TARGET: " << setw(10) << btb->roi_miss_target[cpu][i]
         << "	AVERAGE STALL: "
         << (total_stall_cycle_per_branch[i + 1] * 1.0) /
                total_stall_count[i + 1]
         << endl;
...
void print_sim_stats(uint32_t cpu, CACHE *cache) {
  ...
  for (int i = 0; i < 14; i++) {
    ...
    cout << setw(10) << cache->sim_access[cpu][i] << "  HIT: " << setw(10)
         << cache->sim_hit[cpu][i] << "  MISS: " << setw(10)
         << cache->sim_miss[cpu][i] << "  HIT %: " << setw(10)
         << ((double)cache->sim_hit[cpu][i] * 100 / cache->sim_access[cpu][i])
         << "  MISS %: " << setw(10)
         << ((double)cache->sim_miss[cpu][i] * 100 / cache->sim_access[cpu][i])
         << "  MPKI: " << ((double)cache->sim_miss[cpu][i] * 1000 / num_instrs)
         << endl;
  }
  cout << endl;
}
...
#if defined(SHOTGUN_BTB)
void btb_print_sim_stats(uint32_t cpu, SHOTGUN *cache) {
  uint64_t TOTAL_ACCESS = 0, TOTAL_HIT = 0, TOTAL_MISS = 0;
...
#if defined(SHOTGUN_BTB)
void reset_btb_stats(uint32_t cpu, SHOTGUN *btb) {
  for (uint32_t i = 0; i < NUM_TYPES; i++) {
    btb->sim_access[cpu][i] = 0;
    btb->sim_hit[cpu][i] = 0;
    btb->sim_miss[cpu][i] = 0;
    btb->sim_miss_target[cpu][i] = 0;
    total_stall_cycle_per_branch[i] = 0;
    total_stall_count[i] = 0;
  }
}
#endif
...
void finish_warmup() {
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    ...
    ooo_cpu[i].fetch_stage_stalled = 0;
    ooo_cpu[i].no_decode_cycle = 0;
    ;
    ...
    ooo_cpu[i].total_rob_occupancy_at_btb_mispredict = 0;
...
#if defined(MICRO_BTB)
    ooo_cpu[0].BTB.predecoding_something = 0;
    ooo_cpu[0].BTB.to_predecode = 0;
    ooo_cpu[0].BTB.predecode_request_dropped = 0;
#endif
...
#elif defined(FDIPX_BTB)
    reset_cache_stats(i, &ooo_cpu[i].BTB.L1BTB);
    reset_cache_stats(i, ooo_cpu[i].BTB.L2BTB[0]);
#else
  ...
  dispatches_per_cycle.clear();
  rob_full_cycles = 0;
  decode_empty_cycles = 0;
  decode_dispatch_called = 0;
...
void print_deadlock(uint32_t i) {
  ...
  for (uint32_t j = 0; j < queue->SIZE; j++) {
    cout << "[" << queue->NAME << "] entry: " << j
         << " instr_id: " << queue->entry[j].instr_id
         << " prefetch_id: " << queue->entry[j].prefetch_id
         << " rob_index: " << queue->entry[j].rob_index;
    cout << " address: " << hex << queue->entry[j].address
         << " full_addr: " << queue->entry[j].full_addr << dec
         << " type: " << +queue->entry[j].type;
...
void swap_context(uint8_t swap_cpu_0, uint8_t swap_cpu_1) {
  ...
  // Since context-switch penalty is not included, this needs to be done, once
  // it is added, fetch_stall will be 0 as branch mispredict penalty is only 20,
  // while context-switch penalty is in miliseconds
...
void fill_page_table(string trace_name) {
  ...
  string final_trace;
  final_trace = trace_name.substr(
      69, trace_name.size() - 69); // spec(63), server(65), cloudsuite(69)
  cout << "Final trace: " << final_trace << endl;
  string file_name =
      "results_50M/cloudsuite_PT/" + final_trace +
      "-hashed_perceptron-no-no-no-no-no-no-srrip-drrip-1core-cloudsuite.txt";
...
int main(int argc, char **argv) {
  ...
  std::string condfile, basic_block_file;
  ...
  for (int i = 0; i < argc; i++) {
    if (found_traces) {
      ...
      cout << last_dot << endl;
      std::size_t found = string(full_name).find_last_of("/\\");
      std::string tracename = string(full_name).substr(found + 1);
      condfile = tracename.substr(0, tracename.find_first_of(".")) + ".cond";

      if (tracename.find("srv") != -1)
        basic_block_file = "cvp_sota_results_20M/" + tracename +
                           "-hashed_perceptron-no-no-no-no-no-no-no-lru-lru-"
                           "lru-srrip-drrip-lru-lru-lru-1core-cvp_trace.txt";
      else if (tracename.find("server") != -1 || tracename.find("client") != -1)
        basic_block_file = "ipc_sota_results_50M/" + tracename +
                           "-hashed_perceptron-no-no-no-no-no-no-no-lru-lru-"
                           "lru-srrip-drrip-lru-lru-lru-1core.txt";
      else
        basic_block_file = "spec_sota_results_50M/" + tracename +
                           "-hashed_perceptron-no-no-no-no-no-no-no-lru-lru-"
                           "lru-srrip-drrip-lru-lru-lru-1core.txt";
    ...
    /* Below code is required so that, further arguements are not treated as
     * tracefile */
    if (i + 1 < argc && (strcmp(argv[i + 1], "-context_switch") == 0 ||
                         strcmp(argv[i + 1], "-s") == 0)) {
      found_traces = 0;
      knob_context_switch = 1;
      cout << "knob on" << endl;
  ...
  if (knob_context_switch == 1) {
    ...
    // ooo_cpu[i].context_switch_file = popen(ooo_cpu[i].context_switch_string,
    // "r");
    ...
    index = 0;
    while ((fscanf(context_switch_file, "%ld %d %d", &cs_file[index].cycle,
                   &cs_file[index].swap_cpu[0], &cs_file[index].swap_cpu[1])) !=
           EOF) {
      cs_file[index].index = index;
      cout << "print file:" << cs_file[index].index << " "
           << cs_file[index].cycle << "- " << cs_file[index].swap_cpu[0]
           << cs_file[index].swap_cpu[1] << endl;
      index++;
    }
...
#if defined(MICRO_BTB)
    ooo_cpu[i].BTB.condfile = condfile;
#endif

#if defined(SHOTGUN_BTB)
    ooo_cpu[i].BTB.condfile = condfile;
    ooo_cpu[i].BTB.basic_block_file = basic_block_file;
#else
#if defined(DIV_CONQ)
...
#if defined(SHOTGUN_BTB)
    ooo_cpu[i].L1I.BTB = &(ooo_cpu[i].BTB);
#endif
...
#if defined(SHOTGUN_BTB)
    ooo_cpu[i].L2C.BTB = &(ooo_cpu[i].BTB);
#endif
  ...
  while (run_simulation) {
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
      ...
      /* context-switch code
       ...
       //operating index - If the core is involved in context-switch, it stores
       the index of file which has details about the same
       */
      if (cs_index != -1 &&
          (current_core_cycle[i] == cs_file[cs_index].cycle) &&
          (i == cs_file[cs_index].swap_cpu[0] ||
           i == cs_file[cs_index].swap_cpu[1])) {
        // ooo_cpu[i].operating_index = cs_file[cs_index].index;
        // ooo_cpu[i].context_switch = 1;
        ooo_cpu[cs_file[cs_index].swap_cpu[0]].operating_index =
            cs_file[cs_index].index;
        ooo_cpu[cs_file[cs_index].swap_cpu[1]].operating_index =
            cs_file[cs_index].index;
        ooo_cpu[cs_file[cs_index].swap_cpu[0]].context_switch = 1;
        ooo_cpu[cs_file[cs_index].swap_cpu[1]].context_switch = 1;
        cout << "context_switch at cycle" << cs_file[cs_index].cycle
             << " ,i=" << i << cs_file[cs_index].swap_cpu[0] << " "
             << cs_file[cs_index].swap_cpu[1] << endl;
        // read_context_switch_file = 1;	//@v - ready to read next cycle for
        // context-switching
        cs_index++;
        if (cs_index >= index)
          cs_index = -1;
      ...
      if (stall_cycle[i] <= current_core_cycle[i]) {
        ...
        if (ooo_cpu[i].fetch_stall) {
          ooo_cpu[i].fetch_stage_stalled++;
        }

        // Neelu: Checking IFETCH Buffer occupancy as now instructions won't be
        // added directly to ROB.
        if (ooo_cpu[i].FTQ.check_occupancy(ooo_cpu[i].next_instr.ip) &&
            (ooo_cpu[i].fetch_stall == 0)) {
          ooo_cpu[i].read_from_trace();
        }

        if (warmup_complete[i]) {
          total_rob_occupancy += ooo_cpu[i].ROB.occupancy;
          total_decode_occupancy += ooo_cpu[i].DECODE_BUFFER.occupancy;
          total_ftq_occupancy += ooo_cpu[i].FTQ.occupancy;
        }
      }
      ...
      if (show_heartbeat &&
        ...
        int per_variant_size[4][9] = {0};

        for (int i = 0; i < ooo_cpu[0].BTB.L2BTB.NUM_SET; i++) {
          for (int j = 0; j < ooo_cpu[0].BTB.L2BTB.NUM_WAY; j++) {
            int temp = ooo_cpu[0].BTB.L2BTB.block[i][j].variant;
            if (temp < 4) {
              arr[temp]++;
              per_variant_size[temp][ooo_cpu[0]
                                         .BTB.L2BTB.block[i][j]
                                         .data_per_tag.size()]++;
            }
          }
        ...
        for (int i = 0; i < 4; i++) {
          cout << "Heartbeat_Variant_" << i << " : " << arr[i] << " "
               << arr[i] * 1.0 /
                      (ooo_cpu[0].BTB.L2BTB.NUM_SET *
                       ooo_cpu[0].BTB.L2BTB.NUM_WAY)
               << " per_variant_size: ";
          for (int j = 0; j < 9; j++) {
            cout << per_variant_size[i][j] << " ";
            per_variant_size[i][j] = 0;
          }
          cout << endl;
        }

        for (int i = 0; i < 4; i++) {
          cout << "Heartbeat_Variant_" << i << "_evicts ";
          for (int j = 0; j < 4; j++) {
            cout << j << " : " << variantx_evicty[i][j] << " ";
            variantx_evicty[i][j] = 0;
          }
          cout << endl;
        }
...
#elif defined(FDIPX_BTB)
        record_roi_stats(i, &ooo_cpu[i].BTB.L1BTB);
        record_roi_stats(i, ooo_cpu[i].BTB.L2BTB[0]);
#else
...
#elif defined(FDIPX_BTB)
      print_sim_stats(i, &ooo_cpu[i].BTB.L1BTB);
      print_sim_stats(i, ooo_cpu[i].BTB.L2BTB[0]);
#else
  ...
  for (uint32_t i = 0; i < NUM_CPUS; i++) {
    ...
    cout << " Fetch stage stalled: "
         << ooo_cpu[i].fetch_stage_stalled * 100 /
                ((float)ooo_cpu[i].finish_sim_cycle)
         << "  Sim cycles: " << ooo_cpu[i].finish_sim_cycle
         << " fetch stall cycle: " << ooo_cpu[i].fetch_stage_stalled << endl;
    cout << " Empty decode stage: "
         << ooo_cpu[i].no_decode_cycle * 100 /
                ((float)ooo_cpu[i].finish_sim_cycle)
         << "  Sim cycles: " << ooo_cpu[i].finish_sim_cycle
         << " fetch stall cycle: " << ooo_cpu[i].no_decode_cycle << endl;
...
#elif defined(FDIPX_BTB)
    print_roi_stats(i, &ooo_cpu[i].BTB.L1BTB);
    print_roi_stats(i, ooo_cpu[i].BTB.L2BTB[0]);
#else
  ...
  cout << "Average_ROB_Occupancy: "
       << (total_rob_occupancy * 1.0 / ooo_cpu[0].finish_sim_cycle) << endl;
  cout << "Average_Decode_Occupancy: "
       << (total_decode_occupancy * 1.0 / ooo_cpu[0].finish_sim_cycle) << endl;
  cout << "Average_FTQ_Occupancy: "
       << (total_ftq_occupancy * 1.0 / ooo_cpu[0].finish_sim_cycle) << endl;

  int max = 0;
  int sum = 0;
  for (auto it : unique_branch_target) {
    if (it.second.size() > max)
      max = it.second.size();
    sum += it.second.size();
  }

  cout << "Indirect max targets: " << max
       << " Average targets: " << (sum * 1.0) / unique_branch_target.size()
       << endl;

#ifdef PRINT_PAGE_ADDRESS
  for (auto it : branch_type_per_ip)
    cout << "Branch IP: " << hex << it.first << dec
         << " Branch Type: " << it.second << endl;
  for (auto it : instruction_pages)
    cout << "Instruction Page number: " << hex << it << dec << endl;
  for (auto it : data_pages)
    cout << "Data Page number: " << hex << it << dec << endl;
#endif

  uint64_t distance_per_branch_type[8][8] = {0}, count[8][8] = {0},
           total_count[8] = {0};
  uint64_t less[8] = {10, 20, 30, 60, 100, 150, 200, 240};
  for (auto it : reuse_distance_per_branch) {
    uint64_t branch_type = branch_type_per_ip[it.first];
    int index = 0;
    for (; index < 7; index++)
      if (it.second.first <= less[index])
        break;

    distance_per_branch_type[branch_type][index] += it.second.first;
    count[branch_type][index] += it.second.second;
  }

  string type[] = {"NOT_BRANCH: ",          " BRANCH_DIRECT_JUMP: ",
                   " BRANCH_INDIRECT: ",    " BRANCH_CONDITIONAL: ",
                   " BRANCH_DIRECT_CALL: ", " BRANCH_INDIRECT_CALL: ",
                   " BRANCH_RETURN: ",      " BRANCH_OTHER: "};
  for (int i = 1; i < 7; i++) {
    cout << "Branch_type: " << type[i] << " Count Percentage: ";
    total_count[i] = 0;

    for (int j = 0; j < 8; j++)
      total_count[i] += count[i][j];

    for (int j = 0; j < 8; j++)
      cout << ((count[i][j] * 100.0) / total_count[i]) << " ";
    cout << endl;
  }

  for (int i = 1; i < 7; i++) {
    cout << "Branch_type: " << type[i] << " Total Count: ";
    for (int j = 0; j < 8; j++)
      cout << count[i][j] << " ";
    cout << endl;
  }
  ...
  cout << "To Predecode: " << to_predecode
       << " Predecode_something: " << predecoding_something
       << " Conditional: " << conditional_predecode
       << " Direct call: " << direct_call_predecode
       << " Direct Branch: " << direct_branch_predecode << endl;
#endif

  cout << "Number of unique branch instructions: " << branch_type_per_ip.size()
       << endl;

  cout << "Upper 32 bits: " << endl;
  for (auto it : upper_32_bits)
    cout << hex << it << dec << endl;

  cout << "Unique upper 32 bits: " << upper_32_bits.size() << endl;
  cout << "Unique upper 36 bits: " << upper_36_bits.size() << endl;
  cout << "Unique upper 40 bits: " << upper_40_bits.size() << endl;
  cout << "Unique upper 44 bits: " << upper_44_bits.size() << endl;
  cout << "Unique upper 48 bits: " << upper_48_bits.size() << endl;
  ...
  for (int i = 0; i < 4; i++)
    cout << "Variant_" << i << " : " << arr[i] << " "
         << arr[i] * 1.0 /
                (ooo_cpu[0].BTB.L2BTB.NUM_SET * ooo_cpu[0].BTB.L2BTB.NUM_WAY)
         << endl;

#endif

  for (auto it : offset_bits_required) {
    cout << "Offset_" << it.first << " " << it.second.size() << endl;
  }

  cout << "decode_dispatch_called: " << decode_dispatch_called << " "
       << (decode_dispatch_called * 100.0 / ooo_cpu[0].finish_sim_cycle)
       << endl;

  uint64_t cycles_unutilized = 0, cycles_unutilized_3 = 0,
           cycles_unutilized_0 = 0;

  for (auto it : dispatches_per_cycle) {
    if (it.first < 6)
      cycles_unutilized += it.second;
    if (it.first <= 3)
      cycles_unutilized_3 += it.second;
    if (it.first <= 0)
      cycles_unutilized_0 += it.second;

    cout << "Dispatch_width: " << it.first << " " << it.second << " "
         << it.second * 100.0 / decode_dispatch_called << " "
         << it.second * 100.0 / ooo_cpu[0].finish_sim_cycle << endl;
  }

  cout << "Empty_decode_cycles: "
       << ooo_cpu[0].finish_sim_cycle - decode_dispatch_called << " "
       << (100 - (decode_dispatch_called * 100.0 / ooo_cpu[0].finish_sim_cycle))
       << endl;

  cout << "ROB_full_cycles: " << rob_full_cycles << " "
       << rob_full_cycles * 100.0 / ooo_cpu[0].finish_sim_cycle << endl;

  cycles_unutilized -= rob_full_cycles;
  cycles_unutilized_3 -= rob_full_cycles;
  cycles_unutilized_0 -= rob_full_cycles;
  cycles_unutilized += ooo_cpu[0].finish_sim_cycle - decode_dispatch_called;
  cycles_unutilized_3 += ooo_cpu[0].finish_sim_cycle - decode_dispatch_called;
  cycles_unutilized_0 += ooo_cpu[0].finish_sim_cycle - decode_dispatch_called;

  cout << "Starvation_cycles_per_kilo_instr: "
       << cycles_unutilized * 1000.0 / ooo_cpu[0].num_retired << endl;
  cout << "Starvation_cycles_per_kilo_instr_3: "
       << cycles_unutilized_3 * 1000.0 / ooo_cpu[0].num_retired << endl;
  cout << "Starvation_cycles_per_kilo_instr_0: "
       << cycles_unutilized_0 * 1000.0 / ooo_cpu[0].num_retired << endl;

  for (auto it : offsets_per_branch_type) {
    cout << "Branch_type_" << it.first << " ";
    for (auto jt : it.second)
      cout << " (" << jt.first << "," << jt.second.size() << ") ";
    cout << endl;
  }

  map<uint64_t, uint64_t> trigger_size_bins;

  for (auto it : unique_trigger_per_target) {
    trigger_size_bins[it.second.size()]++;
  ...
  cout << "Unique trigger count: " << endl;
  for (auto it : trigger_size_bins) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      cout << "Variant_" << i << "_evicts_" << j << " : "
           << variantx_evicty[i][j] << endl;
    }
  }
#endif

#if defined(SHOTGUN_BTB)
  cout << "UBTB set used: " << ubtb_set_used.size() << endl;
  cout << "CBTB set used: " << cbtb_set_used.size() << endl;
  cout << "RIB set used: " << rib_set_used.size() << endl;

  cout << "UBTB PER SET UNIQUE ADDR: ";
  for (int i = 0; i < UBTB_SET; i++)
    cout << ubtb_unique_addr[i].size() << " ";
  cout << endl;

  cout << "CBTB PER SET UNIQUE ADDR: ";
  for (int i = 0; i < CBTB_SET; i++)
    cout << cbtb_unique_addr[i].size() << " ";
  cout << endl;

  cout << "RIB PER SET UNIQUE ADDR: ";
  for (int i = 0; i < RIB_SET; i++)
    cout << rib_unique_addr[i].size() << " ";
  cout << endl;

  cout << "UBTB PER SET COUNT: ";
  for (int i = 0; i < UBTB_SET; i++)
    cout << ubtb_fill_count[i] << " ";
  cout << endl;

  cout << "CBTB PER SET COUNT: ";
  for (int i = 0; i < CBTB_SET; i++)
    cout << cbtb_fill_count[i] << " ";
  cout << endl;

  cout << "RIB PER SET COUNT: ";
  for (int i = 0; i < RIB_SET; i++)
    cout << rib_fill_count[i] << " ";
  cout << endl;

#elif !defined(FDIPX_BTB)

  cout << "BTB PER SET UNIQUE ADDR: ";
  for (int i = 0; i < L2BTB_SET; i++)
    cout << btb_unique_addr[i].size() << " ";
  cout << endl;

  cout << "BTB PER SET COUNT: ";
  for (int i = 0; i < L2BTB_SET; i++)
    cout << btb_fill_count[i] << " ";
  cout << endl;
  ...
  for (int i = 0; i < 4; i++) {
    CACHE *temp = ooo_cpu[0].BTB.L2BTB[i];
    cout << temp->NAME << " " << temp->NUM_SET << " " << temp->NUM_WAY << endl;
  }
#endif

#if defined(MICRO_BTB)
  cout << "To predecode: " << ooo_cpu[0].BTB.to_predecode << endl;
  cout << "Predecoding something: " << ooo_cpu[0].BTB.predecoding_something
       << endl;
  cout << "Predecode request dropped: "
       << ooo_cpu[0].BTB.predecode_request_dropped << endl;

  for (int i = 0; i < 9; i++) {
    cout << "sizeofblock_" << i << " " << sizeofblock[i] << endl;
  }

  for (int i = 0; i < 4; i++) {
    cout << "Variant_" << i << " Correct: " << correct_prediction[i]
         << " Incorrect: " << incorrect_prediction[i] << " Accuracy: "
         << correct_prediction[i] * 1.0 /
                (correct_prediction[i] + incorrect_prediction[i])
         << endl;
    ;
  }
#endif
