...
vector<uint64_t> regions_accessed,
    total_regions; // Neelu: regions accessed for ideal spatial prefetcher
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
    ...
    if ((cache_type == IS_LLC) &&
      ...
      if (MSHR.entry[mshr_index].instruction) {
        ...
        sim_miss[fill_cpu][MSHR.entry[mshr_index].type + TYPES_SHIFT]++;
        sim_access[fill_cpu][MSHR.entry[mshr_index].type + TYPES_SHIFT]++;
      }
    ...
    if (do_fill) {
...
#endif
#ifndef PERFECT_L1I
      if (cache_type == IS_L1I && (MSHR.entry[mshr_index].type != PREFETCH)) {
        if (PROCESSED.occupancy < PROCESSED.SIZE)
          PROCESSED.add_queue(&MSHR.entry[mshr_index]);
      }
      ...
      if (cache_type == IS_L1I) //&& (MSHR.entry[mshr_index].type == PREFETCH))
      {
        BTB->predecode(MSHR.entry[mshr_index].full_virtual_address,
                       MSHR.entry[mshr_index].full_virtual_address);
      }
#endif

#endif
#ifndef PERFECT_L1D
      if ((cache_type == IS_L1D) && (MSHR.entry[mshr_index].type != PREFETCH)) {
  ...
  if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) &&
    ...
    if (way >= 0) { // writeback hit (or RFO hit for L1D)
      ...
      if (WQ.entry[index].instruction) {
        ...
        sim_hit[writeback_cpu][WQ.entry[index].type + TYPES_SHIFT]++;
        sim_access[writeback_cpu][WQ.entry[index].type + TYPES_SHIFT]++;
      }
      ...
      if (cache_type == IS_L1D) { // RFO miss
        ...
        if (do_fill) {
          ...
          if (WQ.entry[index].instruction) {
            ...
            sim_miss[writeback_cpu][WQ.entry[index].type + TYPES_SHIFT]++;
            sim_access[writeback_cpu][WQ.entry[index].type + TYPES_SHIFT]++;
          }
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
...
#if defined(PERFECT_ITLB)
      if (cache_type == IS_ITLB) {
        bool page_swap;
        block[set][0].data = ooo_cpu[read_cpu].PTW.va_to_pa_ptw(
                                 read_cpu, RQ.entry[index].instr_id, false,
                                 RQ.entry[index].full_addr,
                                 RQ.entry[index].address, &page_swap) >>
                             LOG2_PAGE_SIZE;
        way = 0;
      }
#endif
      ...
      if (way >= 0) { // read hit
        ...
        if (cache_type == IS_L1I)
          BTB->predecode(RQ.entry[index].full_addr, 0);
#endif
        ...
        if (cache_type == IS_ITLB) {
          ...
          if (temp.l1_pq_index != -1) //@Vishal: Check if the current request is
                                      //sent from L1D prefetcher
          {
        ...
        if (RQ.entry[index].instruction) {
          ...
          sim_hit[read_cpu][RQ.entry[index].type + TYPES_SHIFT]++;
          sim_access[read_cpu][RQ.entry[index].type + TYPES_SHIFT]++;
        }
        ...
        if ((mshr_index == -1) &&
          ...
          if (cache_type == IS_LLC) {
            ...
            if (cache_type == IS_L1I || cache_type == IS_L1D) //@Vishal: VIPT
            {
              ...
              //@Vasudha: Perfect DTLB Prefetcher: add to processed queue even
              //on miss
    ...
    if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) &&
      ...
      if (way >= 0) { // prefetch hit
...
#if defined(SHOTGUN_BTB) && !defined(PERFECT_BTB)
        if (cache_type == IS_L1I)
          BTB->predecode(PQ.entry[index].full_addr, 0);
#endif
        ...
        if (PQ.entry[index].instruction) {
          ...
          sim_hit[prefetch_cpu][PQ.entry[index].type + TYPES_SHIFT]++;
          sim_access[prefetch_cpu][PQ.entry[index].type + TYPES_SHIFT]++;
        }

#if defined(DIV_CONQ)
        if (cache_type == IS_L1I)
          l1i_prefetcher_prefetch_hit(prefetch_cpu, PQ.entry[index].ip,
        ...
        if (PQ.entry[index].pf_origin_level < fill_level) {
          if (cache_type == IS_L1D) {
            ...
            PQ.entry[index].pf_metadata = l2c_prefetcher_operate(
                ...
                1, PREFETCH,
                PQ.entry[index].pf_metadata); // PQ.entry[index].prefetch_id);
...
#if defined(SHOTGUN_BTB) && !defined(PERFECT_BTB)
              // BTB->predecode( PQ.entry[index].address, 0);
#endif
            }
                ...
                if (PQ.entry[index].pf_origin_level < fill_level) {
                  if (cache_type == IS_L1D)
                    ...
                    PQ.entry[index].pf_metadata = l2c_prefetcher_operate(
                        ...
                        PQ.entry[index].ip, 0, PREFETCH,
                        PQ.entry[index]
                            .pf_metadata); // PQ.entry[index].prefetch_id);
                    if ((((PQ.entry[index].pf_metadata >> 17) & 1) |
                         ((PQ.entry[index].pf_metadata >> 18) & 1)) == 1)
                      getting_hint_from_l2++;
                  } else if (cache_type == IS_ITLB) {
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet) {
  ...
  block[set][way].instruction = packet->instruction;
    ...
    uint32_t prefetch_metadata) /*, uint64_t prefetch_id)*/ // Neelu: commented.
{
  ...
  if (PQ.occupancy < PQ.SIZE) {
    ...
    pf_packet.ip = ip;
    // pf_packet.prefetch_id = prefetch_id;		Neelu: commented, Vasudha was using
    // for debugging. Assigning to zero for now.
