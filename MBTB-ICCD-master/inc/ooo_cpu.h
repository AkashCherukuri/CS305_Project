...
#define BRANCH_MISPREDICT_PENALTY                                              \
  1 //@Vishal: Updated from 20 to be same as new ChampSim, anyway penalty is
    //simulated because of queues
//#define SCHEDULING_LATENCY 6
//#define EXEC_LATENCY 1

#define STA_SIZE (ROB_SIZE * NUM_INSTR_DESTINATIONS_SPARC)

#define DECODE_BUFFER_SIZE 60
...
#define BTB_PREFETCH_BUFFER_SET 32
#define BTB_PREFETCH_BUFFER_WAY 2 // less than BTB_WAY

class BTB_PREFETCH_BUFFER {
public:
  // BLOCK entry[BTB_PREFETCH_BUFFER_SET][BTB_PREFETCH_BUFFER_WAY];
  BLOCK **entry;
  std::string condfile;
  BTB_PREFETCH_BUFFER() {
    entry = new BLOCK *[BTB_PREFETCH_BUFFER_SET];
    for (int i = 0; i < BTB_PREFETCH_BUFFER_SET; ++i) {
      entry[i] = new BLOCK[BTB_PREFETCH_BUFFER_WAY];
      for (int j = 0; j < BTB_PREFETCH_BUFFER_WAY; ++j) {
        entry[i][j].valid = 0;
        entry[i][j].lru = j;
      }
    }
  ...
  /*
  check if present in PFB, if yes add to btb and return filled way, else
  return BTB_WAY
  */
  uint32_t check_hit(uint32_t cpu, uint64_t trigger, uint8_t branch_type);

  // adds entry to PFB if not already present in PFB or BTB
  void add_pfb(uint32_t cpu, uint64_t trigger, uint64_t target);
};
...
public:
  ...
  uint64_t fetch_stage_stalled = 0, no_decode_cycle = 0;

  FETCH_TARGET_QUEUE FTQ{"FETCH_TARGET_QUEUE", FTQ_SIZE};

  // reorder buffer, load/store queue, register file
  CORE_BUFFER DECODE_BUFFER{"DECODE_BUFFER",
                            DECODE_BUFFER_SIZE}; // DECODE_WIDTH*3};
  ...
  uint64_t total_rob_occupancy_at_branch_mispredict,
      total_rob_occupancy_at_btb_mispredict = 0;
  ...
  SHOTGUN BTB;
#elif defined(DIV_CONQ)
  BTB_PREFETCH_BUFFER btb_prefetch_buffer;
  BASELINE_BTB BTB;
#elif defined(SKEWED_BTB)
  SKEWEDBTB BTB;
#elif defined(MICRO_BTB)
  MBTB BTB;
...
#ifdef PUSH_DTLB_PB
  CACHE DTLB_PB{"DTLB_PB",       DTLB_PB_SET,
                DTLB_PB_WAY,     DTLB_PB_SET *DTLB_PB_WAY,
                DTLB_PB_WQ_SIZE, DTLB_PB_RQ_SIZE,
                DTLB_PB_PQ_SIZE, DTLB_PB_MSHR_SIZE};
#endif
  ...
  void read_from_trace(),
      ...
      fetch_instruction(), decode_and_dispatch(), schedule_instruction(),
  ...
  ooo_model_instr *add_to_ftq(ooo_model_instr *arch_instr);
...
#if defined(SHOTGUN_BTB)
  void shotgun_l1i_prefetcher_initialize();
  void shotgun_l1i_prefetcher_branch_operate(uint64_t ip, uint8_t branch_type,
                                             uint64_t branch_target);
  void shotgun_l1i_prefetcher_cache_operate(uint64_t v_addr, uint8_t cache_hit,
...
#if defined(SHOTGUN_BTB)
  void btb_update();
  void fill_btb(uint64_t trigger, uint64_t target, uint32_t branch_type);
#elif !defined(SKEWED_BTB)
  void fill_btb(uint64_t trigger, uint64_t target);
#endif
  void l1i_prefetcher_prefetch_hit(uint64_t v_addr, uint64_t blk_addr);
};
