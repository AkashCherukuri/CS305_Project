...
#define L1I_SET 64 // Defined in shotgun.h
#endif
...
#define LLC_PQ_SIZE NUM_CPUS * 64
...
public:
  ...
  uint64_t pf_requested, pf_issued, pf_useful, pf_useless, pf_fill,
      ...
      pf_lower_level, // Count prefetch request that enters MSHR as new miss(not
                      ...
                      // merged
      pf_dropped, late_prefetch, prefetch_count, unique_region_count,
      region_conflicts, same_page_prefetch_requests,
      cross_page_prefetch_requests, sum_pq_occupancy, pf_pushed_from_L2C,
      l1d_data_hit, l2c_data_hit, llc_data_hit, llc_data_miss,
      getting_hint_from_l2, sending_hint_to_llc, stlb_hints_to_l2,
      l1d_pref_accuracy, l2c_pref_accuracy;
  ...
  int check_hit(PACKET *packet), invalidate_entry(uint64_t inval_addr),
      ...
      prefetch_line(
          uint64_t ip, uint64_t base_addr, uint64_t pf_addr,
          int prefetch_fill_level,
          uint32_t
              prefetch_metadata) /* Neelu: commenting, uint64_t prefetch_id)*/,
  ...
  void add_nonfifo_queue(PACKET_QUEUE *queue,
      ...
      l1d_prefetcher_operate(
          uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type,
          uint8_t critical_ip_flag), //, uint64_t prefetch_id),
  ...
  uint32_t get_set(uint64_t address), get_way(uint64_t address, uint32_t set),
      ...
      lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set,
                 ...
                 uint32_t type);
#if defined(DIV_CONQ)
  void l1i_prefetcher_prefetch_hit(uint8_t cpu, uint64_t v_addr,
                                   uint64_t blk_addr);
#endif
};
