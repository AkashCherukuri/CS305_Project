...
folders =  ['BaseLine',
			'BaseLine_Fully Assoc',
            'BaseLine_Direct Mapped']
...
file_names = os.listdir('BaseLine_Fully Assoc')
...
for plot_n in range(3):
	...
	dir_mapped = list(data[plot_n,:, 2]/data[plot_n,:,0])
	...
	plt.bar(br1, base, color =colors[1], width = barWidth,
	        edgecolor =colors[1], label =labels[0])
	plt.bar(br2, fully_assoc, color =colors[2], width = barWidth,
	        edgecolor =colors[2], label =labels[1])
	plt.bar(br3, dir_mapped, color =colors[3] , width = barWidth,
	        edgecolor =colors[3], label =labels[2])
	...
	plt.ylim(0.97,1.03) # FOR IPC

	plt.xticks([r + barWidth for r in range(len(base))],
	        pred_idx)

	plt.title("Normalized baseline IPC for "+str(file_idx[plot_n]), fontsize=20)
